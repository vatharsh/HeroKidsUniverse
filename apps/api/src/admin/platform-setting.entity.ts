...
export const SETTING_DEFAULTS: Record<string, PlatformSettingDefinition> = {
  ...
  APPARENT_AGE_QA_ENABLED:                 { value: 'true',            type: 'boolean', description: 'Run a dedicated apparent-age vision check after each page. Catches age drift that face-consistency score misses (an aged-up child can still score 8/10 on face similarity).' },
  APPARENT_AGE_TOLERANCE:                  { value: '3',               type: 'number',  description: 'Max allowed gap (years) between a character\'s canon age and the apparent age estimated from the generated image before triggering a retry.' },
  APPARENT_AGE_RETRY_CAP:                  { value: '2',               type: 'number',  description: 'Max retry attempts for age-drift pages (separate from face-consistency retries).' },
  CHILD_COSTUME_REWRITE_ENABLED:           { value: 'true',            type: 'boolean', description: 'Rewrite costume descriptions for child characters to use child-coded language ("child-sized", oversized fit details) and strip adult-coded terms.' },
  CHILD_COSTUME_REWRITE_AGE_THRESHOLD:     { value: '13',              type: 'number',  description: 'Characters younger than this age (years) have their costume descriptions rewritten to child-coded language. Adults are untouched.' },
  ENABLE_COSTUMED_CANONICAL:               { value: 'false',           type: 'boolean', description: 'Pre-generate one canonical costumed portrait per child character per story (using costume lock + avatar reference). All page edit calls then use this as reference instead of the plain avatar. Adds one image per child character per story.' },
