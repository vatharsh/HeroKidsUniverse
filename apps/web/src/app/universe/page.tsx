...
export default function UniversePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/universes/mine`, { headers: h })
      .then(async (uRes) => {
        ...
        const u = (Array.isArray(data) ? data[0] : data) as Universe;
        if (!u) { router.push("/onboarding"); return; }
        setUniverse(u);
        // Fetch episodes scoped to this universe
        return fetch(`${BASE}/stories?universeId=${u.id}`, { headers: h });
      })
      .then(async (sRes) => {
        if (sRes?.ok) {
  ...
  async function saveHeroTitle() {
    ...
    try {
      ...
      if (res.ok) {
        setUniverse({ ...universe, heroTitle: titleDraft.trim() });
